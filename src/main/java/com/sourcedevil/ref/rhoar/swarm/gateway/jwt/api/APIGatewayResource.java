...
public class APIGatewayResource {
	...
	private RhoarRaffleCreateEntryProxy buildRhoarRaffleCreateEntryUrl(String name, String nickname) {
		Client client = ClientBuilder.newClient();
		System.out.println("hw url: " + rhoarRaffleCreateEntryUrl);
		WebTarget target = client.target(rhoarRaffleCreateEntryUrl+"/"+name+"/"+nickname);
		ResteasyWebTarget restEasyTarget = (ResteasyWebTarget) target;
    ...
    @GET
    @Path("/create-entry/{name}/{nickname}")
    @Produces(MediaType.APPLICATION_JSON)
    public String apiCreateEntry(@PathParam("name") String name, @PathParam("nickname") String nickname) {
    	RhoarRaffleCreateEntryProxy proxy = buildRhoarRaffleCreateEntryUrl(name, nickname);
        String response = proxy.createRaffleEntry();
        return response;
    }

    @GET
    @Path("/secured/create-entry/{name}/{nickname}")
    @Produces(MediaType.APPLICATION_JSON)
    public String apiSecuredCreateEntry(@PathParam("name") String name, @PathParam("nickname") String nickname) {
        RhoarRaffleCreateEntryProxy proxy = buildRhoarRaffleCreateEntryUrl(name, nickname);
        String response = proxy.createRaffleEntry();
        return response;
    }
